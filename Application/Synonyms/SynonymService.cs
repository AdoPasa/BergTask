...
namespace Application.Synonyms
{
    public class SynonymService
    {
        ...
        public async Task<SynonymResponse?> GetByName(string name)
        {
            ...
            var dbEntry = await _dbContext.Synonyms
        ...
        public async Task<SynonymResponse> Add(AddSynonymRequest request, List<MeaningDto> meaningData) 
        {
            if (await DoesExists(request.Name))
            {
                _logger.LogError("Tried to add an existing synonym: {0}", JsonConvert.SerializeObject(request));
                throw new AppException("Synonym/already-added", System.Net.HttpStatusCode.BadRequest);
            }
            ...
            if (request.ParentId.HasValue) 
            {
                ...
                if (parent == null) {
                    ...
                    throw new AppException("Synonym/invalid-parent", System.Net.HttpStatusCode.BadRequest);
                }
            ...
            var dbEntry = new Synonym
            {
                ...
                Meanings = meaningData.Select(md => new Meaning { 
                    Type = md.Type,
                    Definition = md.Definition,
                    Example = md.Example,
                    Phonetics = md.Phonetics,
                    PhoneticsAudio = md.PhoneticsAudio
                }).ToList(),
            };
